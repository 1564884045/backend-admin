...
export default {
    ...
    methods: {
        ...
        async getData() {
            ...
            this.$api.get('position/list', this.queryData).then(_data => {
                console.log(_data);
                if (_data.code === 200) {
        ...
        loadChild(node, resolve) {
            ...
            if (node.level > 0) {
                ...
                this.$get('position/list', query).then(res => {
                    if (res.code === 200) {
        ...
        async statusChange(e, row) {
            ...
            console.log(e);
            console.log(row);
            let _res = await this.$api.post("position", postData);
        ...
        saveAction(postData, tip = '操作') {
            this.$api.post("position", postData).then(res => {
                if (res.code === 200) {
        ...
        del(row) {
            ...
            this.$confirm('此操作将永久删除该条记录, 是否继续?', '提示', {
                ...
                this.$api.del('position/' + row.id).then(_del => {
                    if (_del.code === 200) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
