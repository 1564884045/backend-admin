        ...
        <el-dialog :visible.sync="showAddBox" :title="editTitle" :close-on-click-modal="false">
            <el-form v-if="showAddBox" ref="form" :model="tmpRow" label-width="150px">
                <el-form-item label="图标（可选）：">
                    <image-upload v-model="tmpRow" field="icon" :action="uploadUrl"/>
                </el-form-item>
...
export default {
    ...
    methods: {
        ...
        async getData() {
            ...
            let _data = await this.$api.get('industry/list', this.queryData);
            console.log(_data);
            if (_data.code === 200) {
        ...
        loadChild(node, resolve) {
            ...
            if (node.level > 0) {
                ...
                this.$get('industry/list', query).then(res => {
                    if (res.code === 200) {
        ...
        async statusChange(e, row) {
            ...
            console.log(e);
            console.log(row);
            let _res = await this.$api.post("industry", postData);
        ...
        saveAction(postData, tip = '操作') {
            this.$api.post("industry", postData).then(res => {
                if (res.code === 200) {
        ...
        del(row) {
            ...
            this.$confirm('此操作将永久删除该条记录, 是否继续?', '提示', {
                ...
                this.$api.del('industry/' + row.id).then(_del => {
                    if (_del.code === 200) {
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
