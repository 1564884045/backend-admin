    ...
    export default {
        ...
        data() {
            return {
                ...
                colsData: [
                    ...
                    {prop: 'content', label: '内容', sortable: false, fixed: false},
        ...
        methods: {
            ...
            async getData() {
                ...
                let _data = await this.$api.get('circle/list', this.queryData);
                console.log(_data)
                if (_data.code === 200) {
            ...
            async statusChange(e, row) {
                ...
                console.log(e);
                console.log(row);
                let _res = await this.$api.post("circle", postData);
                if (_res.code !== 200) {
            ...
            async saveUpdate(e) {
                ...
                let _post = await this.$api.post('circle', e);
                if (_post.code === 200) {
            ...
            del(row) {
                this.$confirm('此操作将永久删除该条记录, 是否继续?', '提示', {
                    ...
                    this.$api.del('/circle/' + row.id).then(_del => {
                        if (_del.code === 200) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
