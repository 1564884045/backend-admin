    ...
    export default {
        ...
        data() {
            return {
                ...
                // 表格字段
                colsData: [
                    {prop: 'name', label: '机构名称', sortable: false, fixed: false},
                    {
                        prop: 'avatar', label: '头像', sortable: false, fixed: false, type: 'img',
                        pipe: (val) => {
                    ...
                    {
                        prop: 'areaText', label: '授课区域', sortable: false, fixed: false,
                        pipe: function (val) {
                            return val.join(",");
                        }
                    }, {
                        prop: 'stageText', label: '授课阶段', sortable: false, fixed: false,
                        pipe: function (val) {
                            return val.join(",");
                        }
                    }, {
                        prop: 'subjectText', label: '授课科目', sortable: false, fixed: false,
                        pipe: function (val) {
                            return val.join(",");
                        }
                    },
        ...
        methods: {
            ...
            async getData() {
                ...
                let _data = await this.$api.get('jiGou/list', this.queryData);
                console.log(_data)
                if (_data.code === 200) {
            ...
            async statusChange(e, row) {
                ...
                console.log(e);
                console.log(row);
                let _res = await this.$api.post("jiGou", postData);
                if (_res.code !== 200) {
            ...
            async saveUpdate(e) {
                ...
                let _post = await this.$api.post('jiGou', e);
                if (_post.code === 200) {
            ...
            del(row) {
                this.$confirm('此操作将永久删除该条记录, 是否继续?', '提示', {
                    ...
                    this.$api.del('/jiGou/' + row.id).then(_del => {
                        if (_del.code === 200) {
                            this.$message({
                                type: 'success',
                                message: '删除成功!'
