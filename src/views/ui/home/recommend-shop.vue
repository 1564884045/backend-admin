    ...
    <div style="max-width: 1000px;margin: auto">
        ...
        <el-dialog :visible.sync="showAddBox" :title="editTitle" :close-on-click-modal="false">
            <el-form v-if="showAddBox" ref="form" :rules="rules" :model="tmpRow" label-width="150px">
                <el-form-item label="推荐店铺" prop="target">
                    <remote-select api="company/list" show-label="name" v-model="tmpRow" field="target"
...
<script>
    ...
    export default {
        ...
        methods: {
            getData() {
                this.$api.get("company/home/list", {position: 'recommend-shop'}).then(res => {
                    if (res.code === 200) {
            ...
            selectChange(e) {
                console.log(e);
                if (!this.tmpRow.cover) this.tmpRow.cover = e.cover;
                if (!this.tmpRow.title) this.tmpRow.title = e.name;
            },
            ...
            async saveUpdate() {
                ...
                _postData.position = 'recommend-shop';
