<template>
    <div class="upload-container">
        <div v-for="(item, index) in url" :key="index" class="images">
            ...
            <div class="mask">
                <div class="actions">
                    ...
                    <span v-show="url.length > 1" title="左移" :class="{'disabled': index === 0}"
        ...
        <el-image-viewer v-if="imageViewerVisible" :on-close="() => {imageViewerVisible = false}"
...
export default {
    ...
    props: {
        ...
        // 绑定的外界form数据，成功后顺带改变父组件绑定的对象中的属性
        value: {
            type: Object,
            default: () => {
                return {}
    ...
    methods: {
        ...
        remove(index) {
            ...
            if (this.field) this.value[this.field].splice(index, 1);
        },
        // 移动
        move(index, type) {
            // 外界列表
            if (this.field) {
                let pathArr = this.value[this.field];
                if (type === 'left' && index !== 0) {
                    pathArr[index] = pathArr.splice(index - 1, 1, pathArr[index])[0]
                }
                if (type === 'right' && index !== pathArr.length - 1) {
                    pathArr[index] = pathArr.splice(index + 1, 1, pathArr[index])[0]
                }
            }
            // 预览列表
        ...
        onSuccess(res) {
            ...
            if (res.code === '200' || res.code === 200) {
                if (this.field) this.value[this.field].push(res.data.path);
