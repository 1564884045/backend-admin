    ...
    <div class="upload-container">
        ...
        <div v-if="!notip" class="el-upload__tip">
            <div style="display: inline-block;">
                <el-alert :title="`上传图片支持 ${ ext.join(' / ') } 格式，且图片大小不超过 ${ size }MB，建议图片尺寸为 ${width}*${height}`"
                          type="info" show-icon :closable="false"/>
            </div>
        </div>
        <el-dialog :modal-append-to-body="true" :visible.sync="dialogVisible" title="预览" width="800px">
...
export default {
    ...
    created() {
        let _url = this.value[this.field];
        if (_url) {
            if (_url.indexOf("http")> -1) this.url = _url;
            else this.url = this.$baseUrl + _url;
        }
    },
    methods: {
        // 移除
        remove() {
            if (this.field) this.value[this.field] = '';
            this.url = '';
        },
        beforeUpload(file) {
        ...
        onSuccess(res) {
            ...
            if (res.code === '200' || res.code === 200) {
                if (this.field) this.value[this.field] = res.data.path;
